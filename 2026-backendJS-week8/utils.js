...
function getDaysAgo(timestamp) {
  ...
  // 3. 用 .diff() 計算天數差異
  const today = dayjs();
  const days = today.diff(dayjs.unix(timestamp), 'day');
  if (days === 0) return '今天';
  return `${days} 天前`;
}
...
function validateOrderUser(data) {
  // 請實作此函式
  const errors = [];
  if (!data.name) errors.push('name 不可為空');
  if (!/^09\d{8}$/.test(data.tel)) errors.push('tel 必須是 09 開頭的 10 位數字');
  if (!data.email.includes('@')) errors.push('email 必須包含 @ 符號');
  if (!data.address) errors.push('address 不可為空');
  if (!['ATM', 'Credit Card', 'Apple Pay'].includes(data.payment)) errors.push('payment 必須是 ATM、Credit Card、Apple Pay 其中之一');
...
function validateCartQuantity(quantity) {
  // 請實作此函式
  if (!Number.isInteger(quantity) || quantity < 1) return { isValid: false, error: '數量必須是正整數' };
