...
async function getUnpaidOrders() {
  ...
  return orders.filter(o => o.paid === false);
}
...
async function getPaidOrders() {
  ...
  const orders = await fetchOrders();
  return orders.filter(o => o.paid === true);
}
...
function displayOrders(orders) {
  ...
  orders.forEach((order, index) => {
    const o = formatOrder(order);
    console.log('========================================');
    console.log(`訂單 ${index + 1}`);
    console.log('----------------------------------------');
    console.log(`訂單編號：${o.id}`);
    console.log(`顧客姓名：${o.user.name}`);
    console.log(`聯絡電話：${o.user.tel}`);
    console.log(`寄送地址：${o.user.address}`);
    console.log(`付款方式：${o.user.payment}`);
    console.log(`訂單金額：${o.totalFormatted}`);
    console.log(`付款狀態：${o.paidText}`);
    console.log(`建立時間：${o.createdAt} (${o.daysAgo})`);
    console.log('----------------------------------------');
    console.log('商品明細：');
