...
function addToCart(carts, product, quantity) {
  ...
  const index = newCarts.findIndex(item => item.product.id === product.id);

  // 邏輯判斷
  if (index !== -1) {
    // 產品已存在，更新數量
    newCarts[index] = { ...newCarts[index], quantity: newCarts[index].quantity + quantity };
  } else {
    // 不存在，新增一筆
...
function updateCartItemQuantity(carts, cartId, newQuantity) {
  ...
  return carts.map(item => item.id === cartId ? { ...item, quantity: newQuantity } : item);
...
function generateOrderReport(orders) {
  ...
  if (totalOrders === 0) return { totalOrders: 0, paidOrders: 0, unpaidOrders: 0, totalRevenue: 0, averageOrderValue: 0 };

  const paidOrders = orders.filter(order => order.paid).length;
  const unpaidOrders = totalOrders - paidOrders;
  const totalRevenue = calculateTotalRevenue(orders);
  
  // 計算所有訂單的總額（不論是否付款）來算平均
  const allOrdersSum = orders.reduce((total, order) => total + order.total, 0);
  const averageOrderValue = Math.round(allOrdersSum / totalOrders);
